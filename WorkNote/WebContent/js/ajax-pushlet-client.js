 ...
 * The most important one is onPushData(). If onEvent() is available that catches
...
// Cross-browser add event listener to element
	...
	_doCallback: function(event, cbFunction) {
		...
		if (cbFunction) {
			// Do specific callback like onPushData(), onJoinAck() etc.
			cbFunction(event);
...
/** Handle incoming events from server. */
	_onEvent: function (event) {
		...
		if (eventType == 'data') {
			PL._setStatus('data');
			PL._doCallback(event, window.onPushData);
